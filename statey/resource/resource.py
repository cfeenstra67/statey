...
from statey.plan import Update
from statey.schema import SchemaSnapshot, Reference, Schema, SchemaHelper
...
class ResourceMeta(abc.ABCMeta):
	...
	def __new__(cls, name: str, bases: Tuple[Type, ...], attrs: Dict[str, Any]) -> 'ResourceMeta':
		...
		if 'type_name' not in attrs:
			raise exc.InitializationError(f'type_name must be defined for every new Resource subclass.')
...
class Resource(abc.ABC, metaclass=ResourceMeta):
	...
	def __init__(self, *args, **kwargs) -> None:
		...
		self.snapshot._resource = self
		self.snapshot = self.schema_helper.fill_missing_values(self.snapshot)
	...
	@abc.abstractmethod
	def update(self, old: SchemaSnapshot, current: SchemaSnapshot, spec: 'Update') -> SchemaSnapshot:
		"""
