	...
	def validate_name(self, name: str) -> None:
		for reserved_string in [self.pathsep, *self.idx_wrap]:
			if reserved_string in name:
				raise ValueError(f'Key {name} cannot contain reserved string: ({repr(reserved_string)}).')
	...
	def split(self, path: str) -> Sequence[Any]:
		...
		for idx_str in idx_strings:
			...
			if idx_str == self.explode_key:
				...
				start, stop, step = map(lambda x: int(x) if x else None, [start, stop, step])
	...
	def join(self, components: Sequence[Any]) -> str:
		...
		for item, next_item in zip(components, chain(components[1:], [object()])):
			...
			if isinstance(next_item, int):
				out.append(''.join([item, self.idx_wrap_left, str(next_item), self.idx_wrap_right]))
				skip = True
			elif isinstance(next_item, slice):
				def string_value(x):
					return '' if x is None else str(x)
				if next_item.step is not None:
					slice_str = ':'.join(map(string_value, [next_item.start, next_item.stop, next_item.step]))
				else:
					slice_str = ':'.join(map(string_value, [next_item.start, next_item.stop])) or ':'
				out.append(''.join([item, self.idx_wrap_left, slice_str, self.idx_wrap_right]))
				skip = True
			elif next_item is utils.EXPLODE:
				out.append(''.join([item, self.idx_wrap_left, self.explode_key, self.idx_wrap_right]))
				skip = True
			else:
				out.append(item)
