...
from statey.syms import (
    impl, impl as impl_mod, Object, session, type_serializers as ts, utils, encoders, semantics, types
)
...
class ReferenceSerializer(ObjectImplementationSerializer):
    ...
    def get_impl_serializer(
        cls,
        impl: impl.ObjectImplementation,
        type: types.Type,
        registry: "Registry"
...
class FutureSerializer(ObjectImplementationSerializer):
    ...
    def serialize(self, implementation: impl.ObjectImplementation) -> Any:
        ...
        if self.return_type_serializer is not None and implementation.return_type is not None:
            out['return_type'] = self.return_type_serializer.serialize(implementation.return_type)
...
class UnknownSerializer(ObjectImplementationSerializer):
    ...
    def serialize(self, implementation: impl.ObjectImplementation) -> Any:
        ...
        if implementation.return_type is not None and self.return_type_serializer is not None:
            out['return_type'] = self.return_type_serializer.serialize(implementation.return_type)
    ...
    def deserialize(
        ...
        return_type = None
        if deserialized['return_type'] is not None and self.return_type_serializer is not None:
            return_type = self.return_type_serializer.deserialize(deserialized['return_type'])
...
class StructSchema(ma.Schema):
    ...
    fields = ma.fields.List(ma.fields.Nested(StructFieldSchema()), required=True, default=list)
...
class DataValueSerializer(DataSerializer):
    ...
    def serialize(self, implementation: impl.ObjectImplementation) -> Any:
        ...
        if implementation.value_type is not None and self.value_type_serializer is not None:
            out['value_type'] = self.value_type_serializer.serialize(implementation.value_type)
    ...
    def deserialize(
        ...
        value_type = None
        if deserialized['value_type'] is not None and self.value_type_serializer is not None:
            value_type = self.value_type_serializer.deserialize(deserialized['value_type'])
            ...
            if implementation.value is None:
                ...
                for item in encoded_value:
                    if isinstance(item, Object):
                        ...
                        serialized = self.element_serializer.serialize(new_impl)['value']
            ...
            if value_list is None:
                ...
                for item in value_list:
                    if 'object' in item:
                        ...
                        sub_impl = self.element_serializer.deserialize({'value': item['data']}, session, objects)
...
class DataStructSerializer(DataSerializer):
    ...
    def get_impl_serializer_from_data(
        ...
        for field in typ.fields:
            ...
            field_serializers[field.name] = registry.get_impl_serializer_from_data(field_data)
...
class FunctionCallSchema(ma.Schema):
    ...
    arguments = ma.fields.Dict(keys=ma.fields.Str(), values=ma.fields.Field(), missing=dict)
...
class FunctionCallSerializer(ObjectImplementationSerializer):
    ...
    def deserialize(
        ...
        loaded_kwargs = {key: objects[obj_id] for key, obj_id in loaded_data['arguments'].items()}
