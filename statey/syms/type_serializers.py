...
class ValueTypeSerializer(TypeSerializer):
	...
	@classmethod
	@st.hookimpl
	def get_type_serializer(cls, type: types.Type, registry: 'Registry') -> 'TypeSerializer':
	...
	@classmethod
	@st.hookimpl
	def get_type_serializer_from_data(cls, data: Any, registry: 'Registry') -> 'TypeSerializer':
...
class StructTypeSerializer(TypeSerializer):
	...
	def serialize(self, type: types.Type) -> Any:
		...
		for key in ordered_fields:
			...
			fields.append({
				'name': key,
				'type': serializer.serialize(type[key].type)
			})
	...
	def deserialize(self, data: Any) -> types.Type:
		...
		for field in data['fields']:
			...
			fields.append(types.StructField(
				name=field['name'],
				type=serializer.deserialize(field['type'])
			))
	...
	def get_type_serializer_from_data(cls, data: Any, registry: 'Registry') -> 'TypeSerializer':
		...
		field_serializers = {}
		for field in data['fields']:
			field_serializers[field['name']] = registry.get_type_serializer_from_data(field['type'])
		return cls(field_serializers)
