...
@pytest.mark.parametrize('annotation, value, result', [
	(str, 1, '1'),
	(int, '12342', 12342),
	(bool, 'some', True)
])
...
def test_graph_func_resolve(graph):
	...
	sym = st.Func[int](lambda x, y, u, v: (x + u) * (y + v))(
		collection1.attrs.a, collection1.attrs.b,
		collection2.attrs.a, collection2.attrs.b
	)
...
def test_graph_func_resolve_multiple(graph):
	...
	for i in range(2, 10):
		collection = Collection[f'fib_{i}'](
			a=collection.attrs.b,
			b=collection.attrs.a + collection.attrs.b
		)
		graph.add(collection)
...
def test_graph_resolve_func_snapshot(graph):
	...
	collection3 = Collection['c'](a=collection1.attrs.a, b=collection2.attrs.a + collection2.attrs.b)
	graph.add(collection1)
	graph.add(collection2)
...
def test_graph_circular_reference(graph):
	...
	class Dummy(st.Resource):
		...
		class Schema(st.Resource.Schema):
			a = st.Field[int](
				optional=True,
				factory=lambda resource: resource.attrs.b
			)
			b = st.Field[int](
				optional=True,
				factory=lambda resource: resource.attrs.a
			)
...
def test_graph_circular_func_reference(graph):
	...
	class Dummy(st.Resource):
		...
		class Schema(st.Resource.Schema):
			a = st.Field[str](
				optional=True,
				factory=lambda resource: resource.attrs.c
			)
			b = st.Field[str](
				optional=True,
				factory=lambda resource: st.F[[str]](resource.attrs.a)
			)
			c = st.Field[str](
				optional=True,
