...
async def test_json_serializer_load(state, graph, tmpdir):
	...
	loaded = serializer.load(json.dumps({
		'meta': {},
		'resources': {
			'Container:/1': {
				'dependencies': {},
	...
	assert loaded.graph.nodes['Container:/1']['snapshot'] == snapshot_cls(attr1=123, attr2='ABCDEFG.')
	assert loaded.graph.nodes['Container:/1']['exists'] is False
	assert loaded.graph.nodes['Container:/2']['snapshot'] == snapshot_cls(attr1=8, attr2='ABCDEFG.ABCDEFG.')
	assert loaded.graph.nodes['Container:/2']['exists'] is False
	assert loaded.graph.nodes['Container:/3']['snapshot'] == snapshot_cls(attr1=16, attr2='ABCDEFG.HELLO WORLD!')
