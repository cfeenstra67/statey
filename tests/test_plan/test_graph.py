...
async def test_build_graph(state, graph, tmpdir):
	...
	assert plan.graph.nodes['file:/file1']['change'].snapshot.copy(size_bytes=None) == file1.schema_helper.snapshot_cls(
		path=os.path.join(tmpdir, 'file1.txt'),
		data=file1_content,
	...
	assert plan.graph.nodes['file:/file2']['change'].snapshot.copy(size_bytes=None) == file2.schema_helper.snapshot_cls(
		path=os.path.join(tmpdir, 'file2.txt'),
		data=file2_content,
...
async def test_build_graph_apply(state, graph, tmpdir):
	...
	with open(os.path.join(tmpdir, 'file2.txt')) as f:
		assert f.read() == f'This was file 1\'s hash: {hashlib.sha256(b"This is file 1.").hexdigest()}'
...
async def test_build_graph_change_apply(state, graph, tmpdir):
	...
	with open(os.path.join(tmpdir, 'file2.txt')) as f:
		assert f.read() == f'This was file 1\'s hash: {hashlib.sha256(b"This is file 1. This too!").hexdigest()}'
