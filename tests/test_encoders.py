...
@pytest.mark.parametrize('type, check', [
    ...
    pytest.param(
        st.Array[str],
        lambda x: isinstance(x, encoders.ArrayEncoder) and isinstance(x.element_encoder, encoders.StringEncoder),
        id='array'
    ),
    pytest.param(
...
@pytest.mark.parametrize('type, data, result', [
    ...
    pytest.param(st.Struct["a": str, "b": int], {'a': '1', 'b': '2'}, {'a': '1', 'b': 2}, id='struct'),
    pytest.param(
